    ...
    public sealed class SsmClient : ISsmClient
    {
        ...
        public async Task<SignPdfResult> SignPdfAsync(SignPdfRequest req, CancellationToken ct = default)
        {
            try
            {
                ...
                // ---- loại ký/vị trí ----
                form.Add(new StringContent(req.SignType.ToString()), "signType");
                form.Add(new StringContent(req.SignLocationType.ToString()), "signLocationType");
                ...
                if (req.Page.HasValue) form.Add(new StringContent(req.Page.Value.ToString()), "page");
                if (req.PositionX.HasValue) form.Add(new StringContent(req.PositionX.Value.ToString("0.##")), "x");
                if (req.PositionY.HasValue) form.Add(new StringContent(req.PositionY.Value.ToString("0.##")), "y");
                ...
                var resp = await http.PostAsync("/api/sign/pdf", form, ct);
